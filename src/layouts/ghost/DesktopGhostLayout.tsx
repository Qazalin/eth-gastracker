import { Box, Skeleton, Flex, VStack } from "@chakra-ui/react";

export const DesktopGhostLayout = () => {
  return (
    <Flex w="100%" h="100%" borderRadius="10px">
      <VStack w="100%" h="100%" borderRadius="inherit">
        <Box
          borderRadius="inherit"
          w="100%"
          h="20%"
          bg="bg1"
          mb="20px"
          p="20px"
        >
          <Skeleton w="100%" h="100%" />
        </Box>
        <Box borderRadius="inherit" w="100%" h="80%" bg="bg1" p="20px">
          <Skeleton w="100%" h="10%" />
          <Skeleton w="100%" h="80%" mt="30px" />
        </Box>
      </VStack>
      <Box borderRadius="inherit" w="50%" h="100%" ml="20px" bg="bg1" p="20px">
        <Skeleton w="100%" h="40%" />
        <Skeleton mt="30px" w="100%" h="40%" />
      </Box>
    </Flex>
  );
};
