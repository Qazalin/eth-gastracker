...
import { IsolatedLayoutProps, ConfirmationTimeChartProps } from "@etherTrack/types"

/**
 * Illustrates the confirmation time using a bar chart for each gas price range
 * @param data The data from Etherscan
 */
export const ConfirmationChartLayout: React.FC<{ data: IsolatedLayoutProps | undefined }> = ({ data }) => {
  if (data) {
    const barChartData: ConfirmationTimeChartProps = [data.SafeGasEstimate, data.ProposeGasEstimate, data.FastGasEstimate]
